...
- **Adaptive Title Bar Theme**: Native Windows window title bar (DWM) now dynamically syncs with the app's theme (Dark Mode for dark themes, Light Mode for light theme).
- **Multilingual Window Title**: Window title dynamically translates according to the active language setting instead of staying permanently in Chinese.
...
### I18n & System Tray
- **Dynamic Tray Menu Localization**: System tray right-click menu items ("Open Window", "Reset Window Position (Center)", "Exit") now dynamically sync and translate in real-time when switching languages between 🇨🇳 简体中文, 🇰🇷 한국어, and 🇺🇸 English.
...
### UI & Branding
- **Brand New High-Resolution Cyber Search Lens Icon**: Upgraded to Option 2 Icon featuring a sleek futuristic magnifying lens focusing on glowing digital data streams and an electric spark lightning accent with a 100% transparent rounded border.
